...
class UpdateMenuItemRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            $selfId = $this->route('menuItem')?->getKey() ?? $this->route('menu_item')?->getKey() ?? $this->route('menuItem') ?? null;
            if ($this->filled('parent_id') && $selfId && (int) $this->integer('parent_id') === (int) $selfId) {
