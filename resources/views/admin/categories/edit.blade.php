...
@section('title', __('admin.edit_category').' — '.config('app.name'))

@section('breadcrumb', __('admin.edit_category'))
...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="max-w-3xl" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        @include('admin.categories.partials.form', ['category' => $category, 'parentChoices' => $parentChoices])
        <div class="mt-8 flex flex-wrap gap-2">
            @if ($admin->hasPermission('categories.update'))
