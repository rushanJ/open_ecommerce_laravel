...
@section('content')
    <x-admin.page-header :title="__('admin.edit_menu')" :subtitle="$menu->name" />
    ...
    <x-admin.card class="mt-6">
        <form method="POST" action="{{ route('admin.menus.update', $menu) }}" class="space-y-6">
            ...
            <div class="flex items-center justify-between gap-3">
                ...
                <form method="POST" action="{{ route('admin.menus.destroy', $menu) }}" onsubmit="return confirm('{{ __('admin.confirm_delete') }}');">
                    @csrf
                    @method('DELETE')
