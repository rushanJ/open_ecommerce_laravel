...
@section('content')
    <x-admin.page-header :title="__('admin.edit_menu_item')" :subtitle="$menuItem->title" />

    <x-admin.card class="mt-6">
        <form method="POST" action="{{ route('admin.menu-items.update', $menuItem) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-between gap-3">
                ...
                <form method="POST" action="{{ route('admin.menu-items.destroy', $menuItem) }}" onsubmit="return confirm('{{ __('admin.confirm_delete') }}');">
                    @csrf
                    @method('DELETE')
