...
@section('title', __('admin.brands').' — '.config('app.name'))
...
@section('content')
    @php
        ...
        $canCreate = $admin->hasPermission('brands.create');
        $canEdit = $admin->hasPermission('brands.update');
        $canDelete = $admin->hasPermission('brands.delete');
        $statuses = [
            '' => __('admin.filter_all'),
    ...
    <x-admin.page-header :title="__('admin.brands')">
        <x-slot:actions>
            @if ($canCreate)
                <x-admin.button type="link" href="{{ route('admin.brands.create') }}" variant="primary" size="sm">
    ...
    @if ($brands->isEmpty())
        <x-admin.empty-state
            ...
            :action-label="$canCreate ? __('admin.create_brand') : null"
            :action-url="$canCreate ? route('admin.brands.create') : null"
        />
    @else
        <x-admin.table>
                ...
                <tbody class="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                    @foreach ($brands as $brand)
                        <tr class="hover:bg-gray-50/80 dark:hover:bg-gray-800/40">
                            ...
                            <td class="max-w-[12rem] truncate px-4 py-3 text-gray-600 dark:text-gray-300">{{ $brand->slug }}</td>
                            <td class="px-4 py-3">
                                <x-admin.badge :variant="$brand->status === 'active' ? 'success' : 'neutral'">
                                    {{ $brand->status === 'active' ? __('admin.active') : __('admin.inactive') }}
                                </x-admin.badge>
                            </td>
                            <td class="px-4 py-3 text-gray-600 dark:text-gray-300">{{ $brand->sort_order }}</td>
                            <td class="whitespace-nowrap px-4 py-3 text-gray-600 dark:text-gray-300">
                                {{ $brand->created_at?->timezone(config('app.timezone'))->format('Y-m-d H:i') }}
                            </td>
                            <td class="px-4 py-3 text-right">
                                @include('admin.shared.table-actions', [
                                    'canView' => false,
                                    'viewRoute' => null,
                                    'editRoute' => $canEdit ? route('admin.brands.edit', $brand) : null,
                                    'deleteRoute' => $canDelete ? route('admin.brands.destroy', $brand) : null,
                                    'canEdit' => $canEdit,
                                    'canDelete' => $canDelete,
                                ])
                            </td>
