...
@section('title', __('admin.edit_attribute_value').' — '.config('app.name'))

@section('breadcrumb', __('admin.edit_attribute_value'))
...
@section('content')
    ...
    <x-admin.page-header :title="__('admin.edit_attribute_value')">
        <x-slot:actions>
    ...
    <form method="POST" action="{{ route('admin.attributes.values.update', [$attribute, $value]) }}" class="max-w-3xl">
        @csrf
        @method('PUT')
