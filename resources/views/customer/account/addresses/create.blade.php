...
@section('title', __('customer.add_address'))

@section('account_content')
    <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex items-end justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">{{ __('customer.add_address') }}</h1>
        ...
        <form method="POST" action="{{ route('customer.account.addresses.store') }}" class="mt-6 space-y-4">
            @csrf
            @include('customer.account.addresses.partials.form')
